...
// function to find a dog by name and increase its age
const findDogByName = async (req, res) => {
  try {
    ...
    dog.age += 1;
    await dog.save();
    return res.json({ message: "Dog's age increased", dog });
  } catch (error) {
    return res.status(500).json({ error: "Failed to update dog's age" });
  }
};
